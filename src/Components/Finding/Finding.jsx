...
import { DELETE_DATA_LICHCHIEU_MAPHIM, GET_DATA_LICHCHIEU_MAPHIM_SAGA_FIND,  } from '../../Redux/Types/dataRapPhim'
import { DivSeacchPhim, DivDetailSearch, DivFramSearch, Input, ButtonClose, ButtonSubmit, DivDisplay } from '../../StyledComponent/Finding/Finding'
...
class Finding extends Component {
            ...
            if(text){
                if(data){
                    return data?.map((item, index) => {
                        return <DivDetailSearch className="tenPhim" key={index} onClick={(event) => this.onClickDetailFinding(event, item.maPhim)}>{item.tenPhim}</DivDetailSearch>
                    })
                }
            }else{
    ...
    onClickDetailFinding = (event, maPhim) => {
        this.setState({
            phim: { ...this.state.phim, text: event.target.innerHTML, maPhim: maPhim }
        })
            ...
            if(text){
                if(dataFinding){
                    return dataFinding?.map((item, index) => {
                        return <DivDetailSearch className="cumRap" key={index} onClick={(event) => this.onClickDetailFindingCumRap(event, item)}>{item.maHeThongRap}</DivDetailSearch>
                    })
                }
            }else{
                 if(dataDefault){
             ...
             return <DivDetailSearch name="cumRapTable" className="cumRap" style={{textAlign: 'center'}}>Vui lòng chọn phim</DivDetailSearch>
         }
    }

    onClickDetailFindingCumRap = (event, item)=>{
        this.setState({
            cumRap: { ...this.state.cumRap, text: event.target.innerHTML, maRapChieu:item }
        })
    }
        ...
        const {text} =this.state.tenRap
         if (maRapChieu) {
            if(text){
                if(dataFinding){
                    return dataFinding?.map((item, index) => {
                        return <DivDetailSearch className="tenRap" key={index} onClick={(event) => this.onClickDetailFindingTenRap(event, item)}>{item.tenCumRap}</DivDetailSearch>
                    })
                }
            }else{
             ...
             return <DivDetailSearch className="tenRap" style={{textAlign: 'center'}}>Vui lòng chọn phim<br></br>và cụm rạp</DivDetailSearch>
         }
    }
    ...
    filterSameDay = ()=>{
        ...
        const fillterSameDay = rap.lichChieuPhim?.filter((thing,index,self)=>{
            const ngayChieu1 =  Format('dd/MM/yyyy', new Date(thing.ngayChieuGioChieu))

            return index === self.findIndex((t)=>{
                const ngayChieu2 =  Format('dd/MM/yyyy', new Date(t.ngayChieuGioChieu))
        ...
        const {text} =this.state.ngayChieu
         if (rap) {
            if(text){
                if(dataFinding){
                    return dataFinding?.map((item, index) => {
                        return <DivDetailSearch className="ngayChieu" key={index} onClick={(event) => this.onClickDetailFindingNgayChieu(event, item.ngayChieuGioChieu)}>
                ...
                return rap?.map((item, index) => {
                    return <DivDetailSearch className="ngayChieu" key={index} onClick={(event) => this.onClickDetailFindingNgayChieu(event, item.ngayChieuGioChieu)} style={{}}>
                        {Format('dd/MM/yyyy', new Date(item.ngayChieuGioChieu))}
                    </DivDetailSearch>
                })
            }
         } else {
             return <DivDetailSearch className="ngayChieu" style={{textAlign: 'center'}}>Vui lòng chọn phim, cụm rạp và rạp</DivDetailSearch>
         }
    }
    ...
    onClickDetailFindingNgayChieu = (event, ngayChieu)=>{
        ...
        this.setState({
            ngayChieu: { ...this.state.ngayChieu, text: event.target.innerHTML, ngayChieu:ngayChieu, rap: rap }
        })
    }
    ...
    const {text} =this.state.suatChieu
     if (rap) {
        if(text){
            if(dataFinding){
                return dataFinding?.map((item, index) => {
                    return <DivDetailSearch className="suatChieu" key={index} onClick={(event) => this.onClickDetailFindingSuatChieu(event, item.maLichChieu)}>
         ...
         return <DivDetailSearch className="suatChieu" style={{textAlign: 'center'}}>Vui lòng chọn phim, cụm rạp, rạp <br></br>và ngày chiếu</DivDetailSearch>
     }
}

onClickDetailFindingSuatChieu = (event, maLichChieu)=>{
    this.setState({
        suatChieu: { ...this.state.suatChieu, text: event.target.innerHTML, maLichChieu:maLichChieu }
    })
}
                    ...
                    <form className="input-group" onSubmit={this.handleOnSubmit}>
                        ...
                        <DivFramSearch className="col-2">
                            <div className="input-group mb-2">
                                <Input
                                    ...
                                    type="text" className='form-control tenPhim' value={this.state.phim.text} placeholder='Tên phim' name="phim" autoComplete="off" />
                                <div className="input-group-prepend" >
                                    <ButtonClose type="button" onClick={this.handleOnClickDelete} className="input-group-text">X</ButtonClose>
                                </div>
                            </div>
                            <DivSeacchPhim>
                        ...
                        <DivFramSearch className="col-2">
                            <div className="input-group mb-2">
                                <Input
                                    ...
                                    type="text" className='form-control cumRap' value={this.state.cumRap.text} 
                                   
                                    placeholder='Cụm Rạp' name="cumRap" autoComplete="off" />
                                <div className="input-group-prepend" >
                                    <ButtonClose type="button" onClick={this.handleOnClickDeleteCumRap} className="input-group-text">X</ButtonClose>
                                </div>
                            </div>
                        ...
                        <div className="input-group mb-2">
                                <Input
                                    ...
                                    type="text" className='form-control tenRap' value={this.state.tenRap.text} 
                                    
                                    placeholder='Tên Rạp' name="tenRap" autoComplete="off" />
                                <div className="input-group-prepend" >
                                    <ButtonClose type="button" onClick={this.handleOnClickDeleteTenRap} className="input-group-text">X</ButtonClose>
                                </div>
                            </div>
                            <DivSeacchPhim>
                        ...
                        <DivFramSearch className="col-2">
                            <div className="input-group mb-2">
                                <Input
                                    ...
                                    type="text" className='form-control ngayChieu' value={this.state.ngayChieu.text} 
                                    placeholder='Ngày chiếu' name="tenRap " autoComplete="off" />
                                <div className="input-group-prepend" >
                                    <ButtonClose type="button" onClick={this.handleOnClickDeleteNgayChieu} className="input-group-text">X</ButtonClose>
                                </div>
                            </div>
                            <DivSeacchPhim>
                                {this.state.ngayChieu.showTable ? this.renderDataNgayChieu() : ""}
                            </DivSeacchPhim>
                        ...
                        <div className="input-group mb-2">
                                <Input
                                    ...
                                    type="text" className='form-control suatChieu' value={this.state.suatChieu.text} 
                                    placeholder='Suất chiếu' name="tenRap " autoComplete="off" />
                                <div className="input-group-prepend" >
                                    <ButtonClose type="button" onClick={this.handleOnClickDeleteSuatChieu} className="input-group-text">X</ButtonClose>
                                </div>
                            </div>
                            <DivSeacchPhim>
                                {this.state.suatChieu.showTable ? this.renderDataSuatChieu() : ""}
                            </DivSeacchPhim>
                        </DivFramSearch>
                        <DivFramSearch className="col-2">
                            <ButtonSubmit type="submit" className="btn btn-success" >Đặt vé</ButtonSubmit>
                        </DivFramSearch> 
                    </form>
                </div>
            </DivDisplay>
