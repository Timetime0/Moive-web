...
import {
  ...
  DivArrow,
  DivDanhSachPhim,
} from "../../StyledComponent/DanhSachPhim/CarouselDanhSachPhim";
import {
  DivChiTietFrame,
  DivImgChiTiet,
  ImgChiTiet,
...
class DanhSachPhim extends Component {
    ...
    return this.props.arrData.map((phim, index) => {
      return (
        <DivListPhimFrame key={index} className="listPhim__item">
          <DivListPhimItem
            lassName="listPhim-for-hover"
            onClick={() => {
              this.handleOnClickItem(phim);
            }}
          >
            ...
            <DivDatVe className="divDatVe">
              <ButtonListPhim
                className="btn"
                onClick={() => {
                  this.handleOnClickDatVe();
                }}
              >
                MUA VÉ
              </ButtonListPhim>
  ...
  handleChiTietPhim = () => {
    ...
    return (
      <DivChiTietFrame>
        <DivImgChiTiet
          onClick={() => this.handleOnClickPhimChiTiet(phim.maPhim)}
        >
            ...
            <div className="speech-bubble">
              ...
              <p>
                ...
                <span style={{ fontWeight: "bolder" }}>
                  {" "}
                  Ngày phát hành
                </span>: {this.handleDay(phim.ngayKhoiChieu)}
              </p>
              <span>
                <span style={{ fontWeight: "bolder" }}>Đánh giá</span>:{" "}
                {this.handleStar(phim.danhGia, "black", "#ffffff57")}
              </span>
              <p>
                <span style={{ fontWeight: "bolder" }}>Mô tả:</span> {phim.moTa}
              </p>
            </div>
          </DivHoverPopup>
        </DivImgChiTiet>
  ...
  handleStar = (item, color, colorFa) => {
    switch (Math.floor(item / 2)) {
      case 0:
        return (
          <DivColorStar inputColor={color} inputColorFa={colorFa}>
            {" "}
    ...
    return (
      ...
      >
        <div className="listPhim row" style={{ position: "relative" }}>
          <div className="col-6 phim">{this.handleChiTietPhim()}</div>
          <SliderCostum
            ref={(c) => (this.slider = c)}
            {...settings}
            className="col-6"
