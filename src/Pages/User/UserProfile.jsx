...
import {
  ...
  DivFramePer,
  DivFrameTable,
} from "../../StyledComponent/Client/client";
class UserProfile extends Component {
      ...
      return thongTinDatVe.map((item, index) => {
        return (
          <tr key={index}>
            ...
            <td>{item.ngayDat}</td>
            <td>
              {item.danhSachGhe.map((chiTiet, index) => {
                return (
                  <div key={index}>
                    {chiTiet.tenHeThongRap} - {chiTiet.tenRap} - Ghế{" "}
                    {chiTiet.tenGhe}
                  </div>
                );
              })}
  ...
  renderThongTinClient = () => {
    ...
    if (clientReducer) {
      return (
        <DivFramePer className="in4__personal">
          ...
          <DivFrameBook className="in4_booking">
            ...
            <DivFrameTable className="pb-5">
              <table className="table table-striped table-dark">
                ...
                <tbody>{this.renderThongTinDatVe()}</tbody>
              </table>
            </DivFrameTable>
          </DivFrameBook>
        </DivFrameIn4>
