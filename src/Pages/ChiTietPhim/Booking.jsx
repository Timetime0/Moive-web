...
import {
  ...
  DELETE_GHE,
  GET_PHONGVE_SAGA,
} from "../../Redux/Types/Admin/quanLyDatVe";
...
import {
  ...
  TableBooking,
  ImgScreen,
} from "../../StyledComponent/Booking/booking";
...
class Booking extends Component {
    ...
    if (danhSachGhe) {
      return danhSachGhe.map((ghe, index) => {
        return (
          <BtnGhe
            key={index}
            onClick={() => this.handleOnClick(ghe)}
            className={this.handleColor(ghe)}
          >
            {ghe.tenGhe}
          </BtnGhe>
        );
    ...
    if (chiTiet) {
      return (
        <DivFrame className="col-12 text-center">
          ...
          <p>{chiTiet.diaChi}</p>
          <p>
            {chiTiet.tenRap} lúc <span>{chiTiet.gioChieu}</span> ngày{" "}
            <span>{chiTiet.ngayChieu}</span>
          </p>
          <h2>Danh Sách Ghế Bạn Chọn</h2>
          <DivFrameDanhSachChonGhiChu>
            <DivFrameIcon>
              <BtnMau className="daDat"></BtnMau>
              <p>Ghế đã đặt</p>
            </DivFrameIcon>
            <DivFrameIcon>
              <BtnMau className="chuaDat"></BtnMau>
              <p>Ghế chưa đặt</p>
            </DivFrameIcon>
            <DivFrameIcon>
              <BtnMau className="dangChon"></BtnMau>
              <p>Ghế đang chọn</p>
            </DivFrameIcon>
          </DivFrameDanhSachChonGhiChu>
          {this.state.choiceGhe ? (
            <TableBooking>
          ...
          <div className="container-fluid">
            <div className="row">
              ...
              <div className="col-md-4">
                ...
                <div className="row">{this.renderChiTietDat()}</div>
              </div>
            </div>
          </div>
        </div>
  ...
  componentDidMount() {
    ...
    this.setState({
      ...this.state,
      thongTinPhim: this.props.thongTinPhim,
    });
  }
}
