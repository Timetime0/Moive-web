...
import {
  ...
  Btn,
  DivLogo1,
} from "../../StyledComponent/ChiTietPhim/LichChieuPhim";
...
class LichChieuPhim extends Component {
      ...
      return (
        <div key={index} className="col-4 col-md-2 py-3">
          <div>
            <DivFrameLogoRap>
              <ImgLogoRap
                className={this.state.idForActiveLogo === index ? `active` : ""}
                onClick={() => this.handleOnClick(rap.maHeThongRap, index)}
                name={rap.maHeThongRap}
                src={rap.logo}
              />
            </DivFrameLogoRap>
          </div>
    ...
    if (this.state.lichChieuPhim.heThongRapChieu.length !== 0) {
      ...
      return (
        <div className="pt-5 text-center">
          <h2>Không có lịch chiếu phim !!!</h2>
        </div>
      );
    }
  };
  ...
  renderlogo = () => {
    ...
    return cumRapChieuPhim?.map((item, index) => {
      return item.cumRapChieu?.map((cumRapChieu, index) => {
        return (
          <DivLogo1 className="col-2 text-center" key={index}>
            <ImglogoLCP
              className={this.state.index === index ? `active` : ""}
              onClick={() => this.handleOnClickLichChieu(index)}
              src={logo}
              alt="true"
            />
  ...
  renderLichChieu = () => {
    const lichChieu =
      this.state.lichChieuPhim.heThongRapChieu[0].cumRapChieu[
        `${this.state.index}`
      ]?.lichChieuPhim;
        ...
        <tr key={index}>
          ...
          <td>
            ...
            <Btn
              ...
              onClick={() => this.handleOnClickBooking(rap.maLichChieu)}
            >
              Đặt vé
            </Btn>
          </td>
      ...
      <div className="rapPhim">
        <div className="container-lg container-fluid">
          ...
          <DivFrameLogoLCP className="logo mt-2">
            <div className="row">{this.renderLogoRapChieuPhim()}</div>
          </DivFrameLogoLCP>
        </div>
        <div className="container-lg container-fluid">
